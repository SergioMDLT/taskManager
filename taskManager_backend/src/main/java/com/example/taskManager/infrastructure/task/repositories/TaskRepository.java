...
public interface TaskRepository extends JpaRepository<TaskEntity, Integer>{
    ...
    @Query("SELECT COUNT(t) > 0 FROM TaskEntity t WHERE t.user.auth0Id = :auth0Id AND t.priority = :priority")
    boolean existsTaskWithPriority(@Param("auth0Id") String auth0Id, @Param("priority") Integer priority);

    @Query("SELECT MAX(t.priority) FROM TaskEntity t WHERE t.user.auth0Id = :auth0Id")
    ...
    @Modifying
    @Query("UPDATE TaskEntity t SET t.priority = t.priority + 1 WHERE t.user.auth0Id = :auth0Id AND t.priority BETWEEN :start AND :end")
